...
```bash
aws ecs create-service --cluster cluster-with-awscli --service-name clarusshop-service --task-definition clarusshop-task-awscli:1 --desired-count 1 --launch-type "FARGATE" --network-configuration "awsvpcConfiguration={subnets=[subnet-00f54c008b549ccb9],securityGroups=[sg-0c03de7301fab0e3a],assignPublicIp=ENABLED}"
```
